...
void printVertexInPath(Vertexes* vertexes, unsigned int vertexIndex) {
	if (vertexIndex == NE || !vertexes || !vertexes->items) {
		return;
	}

	printVertexInPath(vertexes, vertexes->items[vertexIndex].prevIndex);
...
void printPathToVertex(const char* title, Vertexes* vertexes, unsigned int destinationVertexIndex) {
	printf("%s ", title);
	printVertexInPath(vertexes, destinationVertexIndex);
	printf("\r\n");
}
